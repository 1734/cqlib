...
`uint(width)` 与 `bit_vec(width)` 的位宽必须大于零，否则抛出 `CircuitError`。
...
`Circuit.id` 给出所属线路的身份。低层 IR 构造需要脱离线路直接创建句柄时，可以用 `CircuitId()` 显式构造一个身份，同一组相关句柄必须使用同一个 `CircuitId`。
...
`store(target, value)` 用于向经典变量写入表达式。写入时，`value.ty` 必须与 `target.ty` 完全相同，否则 `store()` 直接抛出 `CircuitError`；如果 `target` 属于其他线路，同样会抛出 `CircuitError`。
...
`Measurement` 通常由 `Circuit.measure()` 或 `Circuit.measure_bits()` 返回；低层 IR 构造也可以用上述形式直接创建。
...
如果需要将测量结果作为条件使用，单比特测量结果可以直接转换为 `Bool`：
...
表达式对象带有 `ty` 属性，返回其静态经典类型，可用于在构造控制流之前核对条件与操作数的类型。
...
| `to_bool()` | 将 `Bit` 转换为 `Bool`，等价于 `bit_to_bool()`。 |
| `bit_vec_to_uint()` | 将 `BitVec` 转换为 `UInt`。 |
| `to_uint()` | 将 `BitVec` 转换为 `UInt`，等价于 `bit_vec_to_uint()`。 |
...
当表达式需要作为 `if_()` 或 `while_()` 条件时，应确保其类型为 `Bool`。如果原始表达式是 `Bit`，可以通过 `to_bool()` 进行显式转换；`UInt` 或 `BitVec` 表达式则需要先用 `extract_bit()` 取出单个 `Bit`，再转换为 `Bool`。
...
| `extract_bits(offset, width)` | 从 `UInt` 或 `BitVec` 中提取连续位区间，结果为指定位宽的 `BitVec`。 |
| `concat(parts)` | 拼接多个 `Bit` 或 `BitVec` 表达式，结果为 `BitVec`。 |
...
- `expr1 == expr2` 是表达式的结构相等比较，判断两个表达式对象是否表示同一段结构；要构造运行时比较的表达式，应使用 `ClassicalExpr.equal()` 等静态方法。
- 支持 `copy`、`deepcopy`。
- `repr` 给出表达式的结构表示，便于调试。
...
`ValueControlBody`、`ValueSwitchCase` 与 `ClassicalControlOp` 都支持 `copy`、`deepcopy`，并带有便于调试的 `repr`。
