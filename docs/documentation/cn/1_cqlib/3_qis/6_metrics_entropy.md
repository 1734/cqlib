...
| `entropy` | `concurrence` | two-qubit `DensityMatrix` | 双比特 concurrence |
| `entropy` | `entanglement_of_formation` | two-qubit `DensityMatrix` | 纠缠形成 |
...
## 双比特 concurrence 和纠缠形成

`concurrence()` 和 `entanglement_of_formation()` 针对双比特密度矩阵。
...
这两个指标适合双比特纠缠分析。对于更多 qubit 的系统，应改用纠缠熵、负性、对数负性或按子系统划分的其他指标。
