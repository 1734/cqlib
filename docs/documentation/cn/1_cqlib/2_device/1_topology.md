...
- 
neighbors_undirected(q) 合并两个方向，重复节点只出现一次
...
**边界条件**：
- add_qubits([...])：添加已有比特会抛出 ValueError
- add_couplings([...])：如果耦合端点不在拓扑中，或添加已存在的耦合，抛出 ValueError
- 
remove_qubits([...])：如果比特不存在，抛出 ValueError
- 
remove_couplings([...]) 的 couplings 参数使用 (control, target) 二元组（**不含名称**）
