...
**说明**：
- add_single_qubit_error(gate, qubit, noise) 的 gate 参数传入 StandardGate 枚举值（如 StandardGate.X），而非 Instruction 对象
- add_two_qubit_error(gate, q0, q1, noise) 同样使用 StandardGate 枚举值
- OperationKey.new_single(gate, q0) 和 
new_double(gate, q0, q1) 使用 StandardGate
